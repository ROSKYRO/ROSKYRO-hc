...
import { GROK_PROVIDERS, type SignInProvider } from "./providers";
...
/**
 * The sign-in buttons to actually render, with each one's real mode (native
 * Google/X app vs. the Grok broker) as decided server-side — see
 * `src/lib/server/auth-providers.ts`. Starts empty (no flash of buttons that
 * then turn out unusable) and fills in once the request resolves.
 */
export function useSignInProviders(): { providers: SignInProvider[]; loading: boolean } {
  const [providers, setProviders] = useState<SignInProvider[]>([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    let cancelled = false;
    import("@/lib/server/auth-providers")
      .then(({ getSignInProviders }) => getSignInProviders())
      .then((res) => {
        if (!cancelled) setProviders(res.providers);
      })
      .catch(() => {
        /* leave empty — no working sign-in provider is a valid state */
      })
      .finally(() => {
...
export async function signIn(
  providerId: string,
  opts: { callbackURL?: string; errorCallbackURL?: string; mode?: "native" | "broker" } = {},
): Promise<void> {
  const callbackURL = opts.callbackURL ?? "/";
  const errorCallbackURL = opts.errorCallbackURL ?? "/";
  const mode = opts.mode ?? "broker";

  // Native Google/X (this app's own OAuth app, no broker): a plain top-level
  // redirect always works here — there's no live-preview iframe to worry
  // about, since native creds are never present inside Grok Build's sandbox.
  if (mode === "native") {
